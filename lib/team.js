...
var team = [
	...
	member('jdoe', 'John', 'Doe', 'senior','John Doe will be our head of marketing and public relations.'),

	member('asuszek', 'Austin', 'Suszek', 'senior',
		"Austin was a major player in the full stack. He started out adding authentication and statefulness to the app and creating the original routes. He later took on building real-time messaging for both private messaging and class-wide messaging using socket programming. He used jQuery and AJAX to overhaul the class view making it completely user interactive. He has done a lot of work on the database, server side code, and views to achieve these milestones in the app."),

	member('ndelfino', 'Nick', 'Delfino', 'senior',
		'Nick was a full stack developer for the website. On the database Nick built queries that allowed saving profile information, reporting class message content, handling reported content, and retrieving calendar events. Used AJAX and JQuery to send information users entered in the frontend. This is mainly in the profile view where users can save an abundance of information. In the class view, he added the ability to users to report messages that other users sent. Then in the admin view he added the ability to handle this reported content. To accommodate some of this functionality light UI adjustments were done. When not doing this bug testing and fixing were taking place.'),

	member('annecatherin', 'Catherine', 'Feldman', 'senior','Catherine drove the design and front-end development of Classwire. She specified the app’s organization, and high-level functionality and design mockups for every view. She organized the structure of all handlebars views and configured their content using bootstrap. She prioritized usability and performance in the front-end design and ensured proper back-end integration by developing scripts that used the provided data from the server. She also made the Classwire logo. Although she predominantly owned the frontend, her contributions can be seen throughout the app’s codebase.'),

	member('pscarfi', 'Phil', 'Scarfi', 'senior',
		'Phil was mainly a contributor to the backend. He did everything from registering for an ElephantSQL account, to creating the schema, and setting up the database’s tables on a remote server. After the database was set up, queries are needed to have the application retrieve the right information from the remote server. Phil tackled many of the queries found inside of the app. Along with this, he also worked with uploading resources for different classes. This involved working with some frontend libraries for having popups appear and disappear and javascript and JQuery is what came in clutch for this stuff to happen!'),

	
	member('drabelo', 'Dailton', 'Rabelo', 'senior',
		'Dailton was one of the members of our front end team. He created the class mockup. He helped set up the original class, admin, home, messages, and profile handlebars files. Dailton also was responsible for helping develop the Calendar part of the application alongside Catherine, which allows Classwire users to keep track of their assignments and due dates. He helped put out fires in the front-end and helped resolve bugs as they arose. He also worked on the resources section and built a modal popup that the other popups are based off of.'),

	member('bzifkin', 'Ben', 'Zifkin', 'senior','Ben Zifkin is a senior  at the University of Massachusetts Amherst. ' +
		'He is majoring in Computer Science and minoring in Mandarin. Ben spent the summer as part of a dynamic ' +
		'backend team for a start-up in Chicago. He is looking forward to applying ' +
		'his node.js skills to Classwire\'s backend.')
];
 ...
 *
 * @param  {member object} oldMember a member object
 * @return {member object}        a copy of member
 */
function copy(oldMember) {
	// Just return a new member object using the old information.
	return member(oldMember.user, oldMember.fname, oldMember.lname, oldMember.year, oldMember.desc);
}
...
function one(user) {
	if (typeof user !== 'string') {
		return result(false, 'Illegal Argument: user parameter was not a string', []);
	}
	var memberCopy = find(user);
	if (memberCopy === null) {
		return result(false, 'team member not found', []);
	}
	return result(true, 'team member found', [memberCopy]);
}
