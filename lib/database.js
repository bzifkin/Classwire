...
var ProfileInfoQuery = 'SELECT u.fname, u.lname, s.year, s.major, s.biography, s.activities, s.profileurl' +
...
function getConversation(id, cb){
    pg.connect(constr, (err, client, done) => {
        ...
        client.query(ConversationQuery, [id], (err, result) => {
            ...
            // Strip down conversations to usable data in views.
            var strippedConv = [];
            for (var i = 0; i < result.rows.length; i++) {
              var conversation = result.rows[i];
              var fname = from_user.id === id ? to_user.fname : from_user.fname;
              var lname = from_user.id === id ? to_user.lname : from_user.lname;
              strippedConv.push({id: id, fname: fname, lname: lname});
            }

            cb(undefined, strippedConv);
        });
    });
}
...
function getReportedContent(cb){
    pg.connect(constr, (err, client, done) => {
        ...
        // (3) make the query if successful:
        client.query(getReportedContent(), (err, result) => {
            // call done to release the client back to the pool:
            done();
