...
var CreateStudentQuery = "INSERT INTO student(user_id, profile_picture_url) VALUES($1, '/uploads/empty')";
...
var ClassMessagesQuery = 'SELECT m.id, m.message, m.date_sent, m.from_user, u.fname, u.lname' +
...
var GetReportedContentQuery = 'select v.fname as authf, v.lname as authl, t.fname as reportf, t.lname as reportl, ' +
                            'c.message, r.explanation, r.reported_content'+
                            ' from _user t CROSS JOIN _user v, reported_content r, class_messages c'+
...
var DeleteReportedMessageQuery = ' DELETE FROM class_messages'+
                                 ' WHERE id = $1';
...
var ReportContentQuery = 'INSERT INTO reported_content(author, explanation, report_user, reported_content) VALUES($1, $2, $3, $4)';

var GetClassMessageIdQuery = 'SELECT id FROM class_messages WHERE from_user = $1 AND message = $2';
...
function getReportedContent(cb){
    pg.connect(constr, (err, client, done) => {
        ...
        // (3) make the query if successful:
        client.query(GetReportedContentQuery, (err, result) => {
            // call done to release the client back to the pool:
            done();
  ...
  pg.connect(constr, (err, client, done) => {
    ...
    client.query(GetFriendsQuery, [userId], (err, result) => {
      ...
      for (var i = 0; i < result.rows.length; i++) {
        var friend = result.rows[i];
        if (friend.id in proc_results) {
          proc_results[friend.id].course_number += ', ' + friend.course_number;
        } else {
          proc_results[friend.id] = friend;
        }
      }
      var friends = [];
      for (var k in proc_results) {
        if (Object.prototype.hasOwnProperty.call(proc_results, k)) {
          friends.push(proc_results[k]);
        }
      }
...
function reportContent(author, explanation, reported_user, reported_content, cb){
    pg.connect(constr, (err, client, done) => {
        // (2) check for an error connecting:
        if (err) {
            cb('could not connect to the database: ' + err);
        ...
        client.query(ReportContentQuery, [author, explanation, reported_user, reported_content], (err, result) => {
            // call done to release the client back to the pool:
            done();
...
function getClassMessageId(from_user , message ,cb){
    pg.connect(constr, (err, client, done) => {
        // (2) check for an error connecting:
        if (err) {
            cb('could not connect to the database: ' + err);
        ...
        client.query(GetClassMessageIdQuery, [from_user, message], (err, result) => {
            // call done to release the client back to the pool:
            done();
...
exports.getClassMessageId = getClassMessageId;